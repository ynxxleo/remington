<template>
  <main class="page-wrap mt-1" :class="{ 'collapsed': header.collapsed, 'opaque': header.opaque }">
    ...
    <section class="page-topbar">
      <div class="container">
        <div class="flex-row flex-middle flex-space">
          ...
          <!-- control heading -->
          <div class="flex-1 text-clip text-big text-center me-1 if-medium">
          ...
          <div class="text-nowrap">
            ...
            <Dropdown>
              <button slot="trigger" class="form-btn bg-primary-hover bi bi-chevron-down iconLeft">{{ filterLabel }}</button>
              <div slot="list">

                <div class="flex-row flex-top flex-space px-1 mb-1">
                  <div class="flex-1 me-1 form-label bi bi-twitter iconLeft">Twitter Accounts ({{ accountsList.length }})</div>
                  <button v-if="searchHandle" class="text-bright-hover bi bi-list iconLeft" @click="searchHandle = ''">Show all</button>
                </div>

                <div class="tablelist-wrap mb-1">
                  <div class="tablelist-content">
                    <div class="tablelist-row flex-row flex-middle flex-stretch" v-for="a in accountsList" :key="a.handle">
                      <div class="flex-1 text-bright-hover text-clip clickable me-1" @click="applyFilters( '', a.handle )">
                        <span class="text-info bi bi-twitter"></span>
                      ...
                      <button class="bi bi-x-lg text-danger-hover" title="Remove" v-tooltip @click="removeTwitterHandler( a.handle )"></button>
                    </div>
                  </div>
                </div>

                <form class="twitter-accounts-form px-1 mb-1" action="#" autocomplete="off" @submit.prevent="accountFormHandler">
                  <div class="form-input text-nowrap">
                    <div class="bi bi-twitter iconLeft"></div>
                    <input class="flex-1" type="text" name="handle" placeholder="Add twitter @handle..." />
                    <button class="bi bi-plus-lg text-primary-hover" type="submit"></button>
                  </div>
                </form>

                <div class="text-nowrap px-1">
                  <button class="bi bi-plus-lg iconLeft text-bright-hover" @click="importAccounts()">Import List</button>
                  <span class="text-grey">&nbsp;&nbsp;</span>
                  <button class="bi bi-save iconLeft text-bright-hover" @click="exportAccounts()">Export List</button>
                </div>
            ...
            <Dropdown>
              <button slot="trigger" class="form-btn bg-primary-hover bi bi-gear"></button>
              <div slot="list" class="px-1">

                <div class="mb-1">
                  <div class="form-label mb-1 push-small">News &amp; Notifications Options</div>
                ...
                <div class="mb-1">
                  <div class="form-label mb-1 push-small">How often to send fetch requests</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="1" max="60" step="1" v-model="options.news.interval" @change="saveOptions" />
                    <span class="text-bright">{{ options.news.interval | toNoun( 'sec', 'secs' ) }}</span>
                  </div>
                </div>

                <div class="mb-1">
                  <div class="form-label mb-1 push-small">Delay re-fetching from same source</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="60" max="600" step="10" v-model="options.news.delay" @change="saveOptions" />
                    <span class="text-bright">{{ options.news.delay | toNoun( 'sec', 'secs' ) }}</span>
                  </div>
                </div>

                <div class="mb-1">
                  <div class="form-label mb-1 push-small">Tweets to fetch from each source</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="1" max="10" step="1" v-model="options.news.tweets" @change="saveOptions" />
                    <span class="text-bright">{{ options.news.tweets | toNoun( 'tweet', 'tweets' ) }}</span>
                  </div>
                </div>

                <div class="mb-1">
                  <div class="form-label mb-1 push-small">Limit tweets by days posted</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="1" max="30" step="1" v-model="options.news.days" @change="saveOptions" />
                    <span class="text-bright">{{ options.news.days | toNoun( 'day', 'days' ) }}</span>
                  </div>
                </div>

                <div class="mb-1">
                  <div class="form-label mb-1 push-small">Total number of tweets to store</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="10" max="200" step="10" v-model="options.news.total" @change="saveOptions" />
                    <span class="text-bright">{{ options.news.total }}</span>
                  </div>
                </div>

                <div>
                  <div class="form-label mb-1 push-small">Limit visible tweets on page</div>
                  <div class="flex-row flex-middle flex-stretch">
                    <input class="flex-1 me-1" type="range" min="10" max="100" step="10" v-model="options.news.max" @change="saveOptions" />
        ...
        <div class="card tablelist-wrap">
          ...
          <div class="tablelist-header">
            <div class="flex-row flex-middle flex-stretch">
              ...
              <div class="tablelist-10 text-nowrap me-1 if-small">
              ...
              <div class="tablelist-10 text-nowrap text-end me-1 if-small">
              ...
              <div class="tablelist-30 text-nowrap text-grey me-1 if-medium">
              ...
              <div class="tablelist-10 text-nowrap text-grey text-end">
                Details
              </div>
            </div>
          </div>
          ...
          <div class="tablelist-content py-1 text-center text-info" v-if="!chartData.length">
            <div class="bi bi-graph-up iconLarge"></div>
            <div>Sentiment analysis data for each token based on loaded tweets.</div>
          ...
          <div class="tablelist-content" v-if="chartData.length">
            <div class="tablelist-row flex-row flex-middle flex-stretch" v-for="d in chartList" :key="d.token">
              <div class="tablelist-20 text-clip text-primary me-1">
                <button title="Search" @click="applyFilters( d.search, '' )" v-tooltip><i class="bi bi-search text-info"></i> {{ d.name }}</button>
              </div>
              <div class="tablelist-10 text-clip text-secondary me-1 if-small">{{ d.token }}</div>
              <div class="tablelist-10 text-nowrap text-end me-1 if-small">{{ d.count }}</div>
              <div class="tablelist-30 text-nowrap me-1 if-medium">
                <div v-if="d.barPercent" class="percent-bar">
                  <div :class="d.barColor" :style="{ 'width': d.barPercent +'%' }"></div>
                </div>
              </div>
              <div class="tablelist-20 text-nowrap me-1" :class="d.styles" v-html="d.sentiment"></div>
              <div class="tablelist-10 text-nowrap text-end">
                <button v-if="d.route" class="bi bi-graph-up iconLeft text-btn bg-info-hover" @click.stop="$bus.emit( 'setRoute', d.route )">Info</button>
              </div>
            </div>
          </div>
          ...
          <div class="tablelist-header">
            <div class="newspage-chart-row flex-row flex-middle flex-stretch text-grey">
              ...
              <div class="text-end">
                <button class="bi bi-x-lg iconLeft text-danger-hover" @click="flushTweets()">Flush Data</button>
              </div>
            </div>
          </div>
    ...
    <!-- fallback section -->
    <section class="mb-1" v-if="!tweetsList.length">
      <div class="container">
        <div class="card p-1 flex-row flex-middle flex-stretch">
          <div class="bi bi-info-circle iconLarge text-grey me-1"></div>
          <div class="flex-1">
            <div v-if="searchStr">
    ...
    <section class="pagelist-wrap" id="newspage-list">
      <div class="container">
        ...
        <div class="pagelist-item flex-row flex-top flex-stretch" v-for="t in tweetsList" :key="t.id">
          <div class="me-1" :class="{ 'alert-bubble': t.isNew }">
            <img class="img-round" :src="t.avatar" width="68" height="68" :alt="t.handle" />
          </div>
          <div class="flex-1">
            <div class="flex-row flex-space mb-1">
              ...
              <div class="text-clip if-small">
                ...
                <button type="button" class="bi bi-x-lg text-danger-hover" title="Delete" @click="deleteTweet( t.id )" v-tooltip></button>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    sentimentInfoText() {
      ...
      if ( this.options.news.enabled && !this.priceData.length ) {
        return 'Currently waiting for tokens to load from the socket API...';
      }
      ...
      if ( !this.options.news.enabled && !this.newsEntries.length ) {
        return 'No tweets loaded, use the <i class="bi bi-config"></i> Gear icon to enable fetching.';
      }
    },
  },
  ...
  methods: {
    ...
    updateChart() {
      ...
      // build unique list of tokens from api
