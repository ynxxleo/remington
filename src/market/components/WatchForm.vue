<template>
  <div class="watchform-overlay sticky-top" :class="{ 'visible': visible, 'overflow': overflow, 'under': under }" @click.stop="close">
      ...
      <div class="watchform-container" ref="watchform">
        <div class="container mx-2">
          ...
          <div class="flex-row flex-middle flex-stretch">
            <div class="flex-1 text-info me-1">
              <span class="text-bright bi bi-gear iconLeft">Presets:</span>
              <button v-for="p in presetsList" :key="p.name" class="ms-1 btn btn-light btn-sm" :class="{ 'text-gain': p.active }" @click.prevent="applyPreset( p.name )">{{ p.name }}</button>
            </div>
            <div class="text-clip">
              <span class="bi bi-eye iconLeft text-primary">{{ countInfo }}</span>
            </div>
          </div>
          ...
          <div class="flex-grid flex-grid-md flex-middle">
            ...
            <div class="form-input text-nowrap">
             ...
             <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.priceType" @change="formChange">
                <option value="change">Price change</option>
                <option value="gain">Price gain</option>
                <option value="loss">Price loss</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.priceChange" @change="formChange" />
              <div class="text-secondary">{{ watchOptions.priceChange | toFixed( 1 ) }}%</div>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.volumeType" @change="formChange">
                <option value="change">Volume change</option>
                <option value="gain">Volume gain</option>
                <option value="loss">Volume loss</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.volumeChange" @change="formChange" />
              <div class="text-secondary">{{ watchOptions.volumeChange | toFixed( 1 ) }}%</div>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.priceCheck" @change="formChange">
                <option value="above">Price above</option>
                <option value="below">Price below</option>
              </SelectMenu>
              <input class="flex-1 me-1 text-light" type="text" placeholder="0.00000000" v-model="watchOptions.price" @keyup="numInput" @change="formChange" />
              <div class="text-secondary">{{ watchOptions.market }}</div>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.volumeCheck" @change="formChange">
                <option value="above">Volume above</option>
                <option value="below">Volume below</option>
              </SelectMenu>
              <input class="text-light" type="text" placeholder="0000" v-model="watchOptions.volume" @keyup="numInput" @change="formChange" />
              <div class="text-secondary me-1">{{ watchOptions.market }}</div>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.timeCheck" @change="formChange">
                <option value="less">Within last</option>
                <option value="more">Wait past</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0" max="60" step="1" v-model="watchOptions.timeLimit" @change="formChange" />
              <div class="text-secondary">{{ watchOptions.timeLimit | toNoun( 'min', 'mins' ) }}</div>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.changeCheck" @change="formChange">
                <option value="above">24h % above</option>
                <option value="below">24h % below</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.change" @change="formChange" />
              <span class="text-secondary">{{ watchOptions.change | toFixed( 1 ) }}%</span>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.volatilityCheck" @change="formChange">
                <option value="above">Volatility above</option>
                <option value="below">Volatility below</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.volatility" @change="formChange" />
              <span class="text-secondary">{{ watchOptions.volatility | toFixed( 1 ) }}%</span>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.dangerCheck" @change="formChange">
                <option value="above">Danger above</option>
                <option value="below">Danger below</option>
              </SelectMenu>
              <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.danger" @change="formChange" />
              <span class="text-secondary">{{ watchOptions.danger | toFixed( 1 ) }}%</span>
            </div>

            <div class="form-input text-nowrap">
              <SelectMenu class="me-1" v-model="watchOptions.filterType" @change="formChange">
                <option value="allow">Only tokens</option>
                <option value="deny">Skip tokens</option>
              </SelectMenu>
              <input class="flex-1 me-1 text-light" type="text" placeholder="TOKEN1 TOKEN2 ..." v-model="watchOptions.filterText" @change="formChange"  />
            </div>
            ...
            <div>
              <button
                type="button"
                class="btn iconLeft"
                :class="{ 'btn-danger bi bi-stop-circle': active, 'btn-success bi bi-caret-right-fill': !active }"
