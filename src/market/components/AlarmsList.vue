<template>
  <section>
    ...
    <form v-if="pairData.symbol" class="flex-row flex-middle flex-stretch mb-1" action="#" @submit.prevent="saveAlarm">
      <div class="form-input flex-1 me-1" style="background:rgb(255 255 255 / 6%);">
        <i class="bi bi-graph-up me-1 text-dark"></i>
        <input class="me-1 text-dark" placeholder="0.00000000" name="price" v-model="curPrice" />
        <div class="text-dark">{{ pairData.asset }}</div>
      </div>
      <button type="submit" class="btn btn-sm btn-success bi bi-plus-lg iconLeft"> Set Alarm</button>
    </form>

    <div class="flex-list">
      <div class="flex-header">
        <div class="me-1"><span class="bi bi-alarm text-faded"></span></div>
        ...
        <div><button class="bi bi-x-lg text-danger-hover" title="Delete All" @click="flushAlarms" v-tooltip></button></div>
      </div>
      ...
      <div v-for="a in alarmsList" :key="a.id" class="flex-item">
        <div class="me-1">
          <button class="bi bi-alarm" :class="{ 'text-gain': a.active, 'text-secondary': !a.active }" title="Toggle" @click="toggleAlarm( a.id, a.symbol, !a.active )" v-tooltip></button>
        </div>
        <div class="flex-1 me-1">
          <button class="text-dark" @click="$bus.emit( 'setRoute', '/symbol/'+ a.symbol )">{{ a.pair }}</button>
        </div>
        <div class="flex-1 me-1">
          <span class="text-big" :class="[ 'text-'+ a.check ]">{{ a.sign }}</span>
          <span class="text-dark">{{ a.price | toFixed( a.asset ) }}</span>
          <span class="text-secondary">{{ a.asset }}</span>
        </div>
        <div class="flex-1 me-1">
          <span :class="{ 'text-success': a.active, 'text-secondary': !a.active }">{{ a.active ? 'Active' : 'Triggered' }}</span>
        </div>
        <div class="flex-1 me-1">
          <span class="text-grey">{{ a.time | toDate }}</span>
        </div>
        <div>
          <button class="bi bi-x-lg text-dark" title="Delete" @click="deleteAlarm( a.id, a.symbol )" v-tooltip></button>
        </div>
      </div>
