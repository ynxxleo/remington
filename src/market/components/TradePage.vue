    ...
    <section class="page-topbar">
      <div class="container">
        <div class="flex-row flex-middle flex-space">
          ...
          <!-- control search -->
          <Search class="light me-1" v-model="searchStr"></Search>

          <!-- control heading -->
          <div class="flex-1 text-clip text-big text-center me-1 if-medium">Trading Bot</div>

          <!-- control dropdown menus -->
          <Dropdown>
            <button slot="trigger" class="form-btn bg-primary-hover icon-down-open iconLeft">
              <span v-if="socketStatus === 0">API Offline <i class="bi bi-x-lg"></i></span>
            ...
            <div slot="list" class="text-center">
              <div class="form-label px-1">Account API Connection</div>
              <hr />
              <div class="px-1 mb-1">
              ...
              <div class="px-1" :class="{ 'disabled': !canConnect }">
                <button v-if="socketStatus === 0" class="form-btn bg-success-hover bi bi-x-lg iconLeft" @click="initUserStream">Connect</button>
                <button v-if="socketStatus === 1" class="form-btn bg-info-hover bi bi-x-lg iconLeft">Waiting...</button>
                <button v-if="socketStatus === 2" class="form-btn bg-danger-hover bi bi-x-lg iconLeft" @click="stopUserStream">Disconnect</button>
              </div>
            </div>
          </Dropdown>
        ...
        <div class="card tablelist-wrap">
          ...
          <div class="tablelist-header">
            <div class="flex-row flex-middle flex-stretch">
              <div class="tablelist-20 text-clip me-1">
              ...
              <div class="tablelist-20 text-nowrap me-1">
              ...
              <div class="tablelist-20 text-nowrap text-end me-1">
              ...
              <div class="tablelist-20 text-nowrap text-end">
          ...
          <div class="tablelist-content py-1 text-center text-info pos-rel" v-if="!balancesList.length">
          ...
          <div class="tablelist-content" v-if="balancesList.length">
            <div class="tablelist-row flex-row flex-middle flex-stretch" v-for="t in balancesList" :key="t.asset">
              <div class="tablelist-20 text-clip text-bright me-1 if-medium">{{ t.name }}</div>
              <div class="tablelist-20 text-nowrap text-bright me-1">
                <button class="text-primary-hover" title="Info" @click="$bus.emit( 'setRoute', t.route )" v-tooltip>{{ t.asset }}</button>
              </div>
              <div class="tablelist-20 text-nowrap text-primary text-end text-monospace me-1">{{ t.total | toFixed }}</div>
              <div class="tablelist-20 text-nowrap text-bright text-end text-monospace me-1">{{ t.free | toFixed }}</div>
              <div class="tablelist-20 text-nowrap text-danger text-end text-monospace">{{ t.locked | toFixed }}</div>
              <div class="tablelist-20 text-nowrap text-end">
      ...
      <div class="container">
        ...
        <div class="card p-1">
          ...
          <div class="flex-row flex-space">
            <div class="text-nowrap text-bright me-1">
              <span class="icon-chart-line iconLeft text-nowrap">Trade Bot</span>
            </div>
            <div class="text-nowrap me-1 if-medium">
            ...
            <div class="text-nowrap me-1 if-small">
          ...
          <div class="flex-row flex-middle flex-srtetch">
            ...
            <div class="flex-1 text-clip me-1">
          ...
          <div class="bot-form pt-1" :class="{ 'visible': formShow }">

            <div class="flex-grid flex-grid-md flex-middle" :class="{ 'disabled': botActive }">

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Currency</span>
                <SelectMenu class="flex-1 me-1" :options="assetOptions" v-model="watchOptions.asset" @change="onBotOptions"></SelectMenu>
                <span class="text-secondary">{{ watchOptions.assetBalance | toFixed( watchOptions.asset ) }}</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Only use</span>
                <input class="flex-1 me-1" type="range" min="5" max="100" step="5" v-model="watchOptions.assetPercent" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.assetPercent }}% {{ watchOptions.asset }}</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Split trades</span>
                <input class="flex-1 me-1" type="range" min="1" max="10" step="1" v-model="watchOptions.assetSplit" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.assetSplit }}</span>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.filterType" @change="onBotOptions">
              ...
              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.priceType" @change="onBotOptions">
                  <option value="change">Price change</option>
                  <option value="gain">Price rise</option>
                  <option value="loss">Price drop</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.priceChange" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.priceChange | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Volume gain</span>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.volumeChange" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.volumeChange | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Within last</span>
                <input class="flex-1 me-1" type="range" min="0" max="60" step="1" v-model="watchOptions.timeLimit" @change="onBotOptions" />
                <div class="text-secondary">{{ watchOptions.timeLimit | toNoun( 'min', 'mins' ) }}</div>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Sell profit</span>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.priceProfit" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.priceProfit| toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Stop loss</span>
                <input class="flex-1 me-1" type="range" min="0.0" max="10.0" step="0.5" v-model="watchOptions.priceStop" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.priceStop | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.priceCheck" @change="onBotOptions">
                  <option value="above">Price above</option>
                  <option value="below">Price below</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="text" placeholder="0.00000000" v-model="watchOptions.price" @change="onBotOptions" />
                <div class="text-secondary">{{ watchOptions.asset }}</div>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.volumeCheck" @change="onBotOptions">
                  <option value="above">Volume above</option>
                  <option value="below">Volume below</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="text" placeholder="0000" v-model="watchOptions.volume" @change="onBotOptions" />
                <div class="text-secondary">{{ watchOptions.asset }}</div>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.changeCheck" @change="onBotOptions">
                  <option value="above">24h % above</option>
                  <option value="below">24h % below</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.change" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.change | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.volatilityCheck" @change="onBotOptions">
                  <option value="above">Volatility above</option>
                  <option value="below">Volatility below</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.volatility" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.volatility | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <SelectMenu class="me-1" :css="'text-primary-hover'" v-model="watchOptions.dangerCheck" @change="onBotOptions">
                  <option value="above">Danger above</option>
                  <option value="below">Danger below</option>
                </SelectMenu>
                <input class="flex-1 me-1" type="range" min="0.0" max="100.0" step="0.5" v-model="watchOptions.danger" @change="onBotOptions" />
                <span class="text-secondary">{{ watchOptions.danger | toFixed( 1 ) }}%</span>
              </div>

              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Limit</span>
              ...
              <div class="form-input text-nowrap">
                <span class="me-1 text-primary">Order type</span>
              ...
              <div v-if="watchOptions.orderType === 'LIMIT'" class="form-input text-nowrap">
                <span class="me-1 text-primary">Execution</span>
        ...
        <div class="card p-1">
          ...
          <Tabs :data="{ totalOrders, totalSessions, pendingTrades, totalTrades }">
            ...
            <section btn-class="icon-chart-line iconLeft" :btn-name="'Bot Trades ('+ pendingTrades +'/'+ totalTrades +')'" active>
              <div class="py-1 text-center text-info" v-if="!tradesList.length">
              ...
              <div class="flex-list" v-if="tradesList.length">
                <div class="flex-header flex-middle flex-space">
                  <div class="iconWidth text-nowrap me-1 if-medium"></div>
                  <div class="flex-10 text-nowrap me-1">
                  ...
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'trades', 'active', 'asc' )">Status</span></div>
                  <div class="flex-10 text-nowrap text-info me-1">Buy Price</div>
                  <div class="flex-10 text-nowrap text-info me-1">Cur/Sell Price</div>
                  <div class="flex-10 text-nowrap text-info me-1">P/L</div>
                  <div class="text-nowrap text-end">
                    <Dropdown>
                      ...
                      <ul slot="list">
                        ...
                        <li class="text-bright-hover bi bi-x-lg iconLeft clickable" @click="flushTradeEntries()">Delete Trade Entries</li>
                      </ul>
                    </Dropdown>
                  </div>
                </div>
                <div class="flex-item flex-middle flex-stretch" v-for="t of tradesList" :key="t.id">
                  <div class="iconWidth text-nowrap me-1 if-medium">
                  ...
                  <div class="text-nowrap text-end">
                    <Dropdown>
                      ...
                      <ul slot="list">
                        ...
                        <li class="text-danger-hover bi bi-x-lg iconLeft clickable" @click="removeTradeEntry( t.id )">Remove from list</li>
                      </ul>
                    </Dropdown>
                  </div>
                </div>
            ...
            <section btn-class="icon-list iconLeft" :btn-name="'Order Activity ('+ totalOrders +')'">
              <div class="py-1 text-center text-info" v-if="!ordersList.length">
              ...
              <div class="flex-list" v-if="ordersList.length">
                <div class="flex-header flex-middle flex-space">
                  ...
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'orders', 'side', 'asc' )">Order</span></div>
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'orders', 'status', 'asc' )">Status</span></div>
                  <div class="flex-10 text-nowrap text-info me-1">Price</div>
                  <div class="flex-10 text-nowrap text-info me-1 if-medium">Fee</div>
                  <div class="flex-10 text-nowrap text-info me-1">Total</div>
                  <div class="text-nowrap text-end">
                    <Dropdown>
                      <button slot="trigger" class="text-primary-hover icon-config"></button>
                      <ul slot="list">
                        <li class="text-bright-hover bi bi-x-lg iconLeft clickable" @click="cancelOpenOrders()">Cancel Open Orders</li>
                ...
                <div class="flex-item flex-middle flex-space" v-for="o of ordersList" :key="o.id">
                  ...
                  <div class="flex-10 text-warning text-nowrap me-1">
                  ...
                  <div class="flex-10 text-nowrap me-1 if-medium">
                  ...
                  <div class="text-nowrap text-end">
                    <Dropdown>
                      ...
                      <ul slot="list">
                        ...
                        <li class="text-danger-hover bi bi-x-lg iconLeft clickable" @click="removeOrder( o.id )">Remove from list</li>
                      </ul>
                    </Dropdown>
                  </div>
                </div>
            ...
            <section btn-class="icon-calendar iconLeft" :btn-name="'Bot Sessions ('+ totalSessions +')'">
              <div class="py-1 text-center text-info" v-if="!sessionsList.length">
                <div class="icon-calendar iconLarge"></div>
                <div>No bot sessions data available.</div>
              </div>
              <div class="flex-list" v-if="sessionsList.length">
                <div class="flex-header flex-middle flex-space">
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'pair', 'asc' )">Pair</span></div>
                  <div class="flex-10 text-nowrap me-1 if-medium"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'time', 'desc' )">Time</span></div>
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'balance', 'desc' )">Balance</span></div>
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'complete', 'desc' )">Trades</span></div>
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'profit', 'desc' )">Profit</span></div>
                  <div class="flex-10 text-nowrap text-info me-1">Duration</div>
                  <div class="flex-10 text-nowrap me-1"><span class="clickable" @click="$sorter.sortOrder( 'sessions', 'live', 'desc' )">Live</span></div>
                  <div class="text-nowrap text-end">
                    <button class="text-bright-hover bi bi-x-lg" title="Flush List" @click="flushSessionData" v-tooltip></button>
                  </div>
                </div>
                <div class="flex-item flex-middle flex-stretch" v-for="s of sessionsList" :key="s.id">
                  ...
                  <div class="text-end">
                    <button class="text-loss-hover bi bi-x-lg" title="Remove" @click="deleteSessionEntry( s.id )" v-tooltip></button>
                  </div>
                </div>
              </div>
            </section>
