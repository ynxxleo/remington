            ...
            <div class="table-responsive">
                <table class="table table-hover custom-data-bs-table">
                    <thead>
                        <tr>
                            <th>{{ __('locale.ID')}}</th>
                            <th>{{ __('locale.Crypto Currency')}}</th>
                            <th>{{ __('locale.Crypto Symbol')}}</th>
                                ...
                                <td data-label="@lang("Status")">
                                 ...
                                 @if($contract->result != NULL)
                                    @if (isset($datas[$user->id][$contract->id]))
                                        @if (Request::is('**/trade*'))
                                        <td><a href="{{ route('user.trade.contract.log.preview', $contract->id) }}" class="btn btn-icon btn-sm btn-outline-info"><i class="bi bi-info-lg"></i></a></td>
                                        @else
                                        <td><a href="{{ route('user.practice.contract.log.preview', $contract->id) }}" class="btn btn-icon btn-sm btn-outline-info"><i class="bi bi-info-lg"></i></a></td>
                                        @endif
                                    @else
