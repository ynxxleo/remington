                ...
                <div class="col text-center">
                    ...
                    <div class="input-group ps-1 w-auto">
                        @if (Request::is('**/practice*'))
                                @if(auth()->user()->practice_balance != 0)
                                ...
                                <button class="btn btn-outline-warning"
                                    ...
                                    <i class="bi bi-plus"></i>Top Up</button>
                                @endif
                            @else
                            <input type="number" class="form-control" min="{{ $limits->min_amount }}" max="{{ $limits->max_amount }}" step="{{ $limits->min_amount }}" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')" required="" id="amount" name="amount" placeholder="Amount">
                            @endif
...
<div class="modal modal-slide-in fade" id="schedule">
    <div class="modal-dialog sidebar-sm">
      <form class="add-new-record modal-content pt-0" @if (Request::is('**/practice*')) action="{{route('user.practice.schedule')}}" @elseif (Request::is('**/trade*')) action="{{route('user.trade.schedule')}}" @endif method="POST" enctype="multipart/form-data">
          @csrf
        ...
        <div class="modal-body flex-grow-1">
            ...
            <input type="hidden" name="account" @if (Request::is('**/practice*')) value="1" @elseif (Request::is('**/trade*')) value="2" @endif>
        ...
        $(document).ready(function () {
            ...
            $("#playGame").on('submit', function (event4) {
                ...
                if (!highLowArray.includes(parseInt(highlowType))) {
                    ...
                    @else
                    notify('error', 'Your Balance {{ getAmount(auth()->user()->balance) }} {{$general->cur_text}} Not Enough! Please Add Balance First');
                @endif
