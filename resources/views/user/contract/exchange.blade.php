...
{{-- <div class="modal modal-slide-in fade" id="schedule">
    <div class="modal-dialog sidebar-sm">
      <form class="add-new-record modal-content pt-0" action="{{route('user.exchange.schedule')}}" method="POST" enctype="multipart/form-data">
          @csrf
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">×</button>
        <div class="modal-header mb-1">
          <h5 class="modal-title" id="send1">{{ __('locale.Stop Market Lose') }}</h5>
        </div>
        <div class="modal-body flex-grow-1">
            <input type="hidden" name="market" value="{{ $currency->symbol }}">
            ...
            @if($fromW != 'null')
                ...
                <div class="input-group mb-1">
                    <input type="number" class="form-control" min="{{ $limits->min_amount }}" max="{{ $limits->max_amount }}"
                     step="{{ $limits->min_amount }}" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')"
            ...
            <div class="input-group mb-1">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                  <li><a class="dropdown-item text-success" onclick="$('#typed').text($(this).text());
                    ...
                    $('#typed').addClass('btn-success');" data-type="1">Buy</a></li>
                  <li><a class="dropdown-item text-danger" onclick="$('#typed').text($(this).text());
                    ...
                    $('#typed').addClass('btn-danger');" data-type="2">Sell</a></li>
                </ul>
            </div>
            <label class="form-label" for="price">{{ __('locale.Minimum Drop In Price To Sell')}}</label>
            <div class="input-group mb-1">
                <input type="text" class="form-control min_price" id="min_price" maxlength="30" name="min_price" value="{{ ttz(getCoinRate($symbol)) }}" placeholder="{{ __('locale.Sell Order When Price = ..')}}" required>
                <span class="input-group-text text-light">{{ $pair }}</span>
            </div>
            <label class="form-label" for="price">{{ __('locale.Maximum Rise In Price To Sell')}}</label>
            <div class="input-group mb-1">
                <input type="text" class="form-control price" id="max_price" maxlength="30" name="price" value="{{ ttz(getCoinRate($symbol)) }}" placeholder="{{ __('locale.Sell Order When Price = ..')}}" required>
                <span class="input-group-text text-light">{{ $pair }}</span>
            </div>
            <button type="submit" class="btn btn-primary me-1" disabled>{{ __('locale.Place Order')}}</button>
            <button type="reset" class="btn btn-outline-secondary" data-bs-dismiss="modal">{{ __('locale.Cancel')}}</button>
            <hr>
            <small class="text-danger">Beta Release, Under Development</small>
        </div>
      </form>
    </div>
  </div> --}}
