    ...
    <div class="col-lg-4 col-md-6 col-sm-12">
        <div class="card card-congratulation-medal mh-22vh">
            <div class="card-body">
                ...
                <a href="{{ $pageObj['url']}}" type="button" class="mt-3 btn btn-primary">{{ $pageObj['btn_name']}}</a>
                        ...
                        @forelse($bot_contracts as $bot)
                            <tr>
                                ...
                                <td data-label="{{ __('locale.Profit/Loses')}}">
                                    @if ($bot->status == 1)
                                        <div class="fw-bold @if ($bot->result == 1) text-success @elseif($bot->result == 2) text-danger @else @endif">@if ($bot->result == 1) + @elseif($bot->result == 2) - @else @endif{{ ttz($bot->profit) }} {{ $crypto ? $bot->pair : "USDT" }}</div>
                                    @else
                                        @if($platform->bot_fake == 1)
                                            ...
                                            <script>
                                                setInterval(function gRI_{{$bot->id}}() {
                                                    ...
                                                    crytpo = {{$crypto ? "true" : "false"}}
                                                    bot_p_{{$bot->id}} = crypto == "true" ? "{{ $bot->pair }}" : "USDT";
