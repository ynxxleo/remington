    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="card-title">{{ __('locale.Deposit History')}}</h4><div class="card-search"></div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($data->admin_feedback != null)
                                        <button class="btn-info btn-rounded  badge detailBtn" data-admin_feedback="{{$data->admin_feedback}}"><i class="bi bi-info-circle"></i></button>
                                    @endif
                                ...
                                <td data-label="{{ __('locale.Details')}}">
                                    <a href="javascript:void(0)" class="btn btn-primary btn-sm approveBtn"
                                       ...
                                       <i class="bi bi-info-circle-circle"></i>
                                    </a>
                                </td>
                            </tr>
                        @empty
        ...
        </div><div class="mb-1">{{paginateLinks($logs) }}</div>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <ul class="list-group text-center list--group">
                    <li class="list-group-item">{{ __('locale.Amount')}} : <span class="withdraw-amount "></span></li>
                    <li class="list-group-item">{{ __('locale.Charge')}} : <span class="withdraw-charge "></span></li>
                    <li class="list-group-item">{{ __('locale.After Charge')}} : <span class="withdraw-after_charge"></span></li>
                    <li class="list-group-item">{{ __('locale.Conversion Rate')}} : <span class="withdraw-rate"></span></li>
                    <li class="list-group-item">{{ __('locale.Payable Amount')}} : <span class="withdraw-payable"></span></li>
                </ul>
...
@push('script')
    <script>
        ...
        $('.approveBtn').on('click', function() {
            ...
            if (singleInfo)
            {
                modal.find('.withdraw-detail').html(`<br><strong class="my-3">{{ __('locale.Payment Information')}}</strong>  ${singleInfo}`);
            }else{
