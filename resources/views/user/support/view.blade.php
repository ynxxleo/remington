      ...
      <div class="card">
        <div class="card-body">
                <div class="card-header">
                    ...
                    <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#DelModal" class="btn-sm d-block btn btn-primary btn-danger text-center mt-2">{{ __('locale.Close Ticket')}}</a>
                </div>
                <div class="card-body">
                @if($my_ticket->status != 4)
                    <form method="post" action="{{ route('ticket.reply', $my_ticket->id) }}" class="message__chatbox__form row" enctype="multipart/form-data">
                        ...
                        <div class="form-group col-sm-12">
                            <textarea id="message" name="message" class="form-control " placeholder="{{ __('locale.Enter Message')}}"></textarea>
                        </div>
                        <div class="form-group col-sm-12">
                            <div class="d-flex mt-1">
                                <div class="start-group col p-0">
                                    <label for="file" class="label">{{ __('locale.Attachments')}}</label>
                                    <input type="file" class="overflow-hidden form-control  mb-2" id="file" name="attachments[]">
                                ...
                                <div class="add-area">
                                    ...
                                    <button class="btn btn-primary btn-sm bg-primary  ms-2 ms-md-4"  onclick="extraTicketAttachment()" type="button"><i class="bi bi-plus"></i></button>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-2">
                                            <div class="reply-thumb">
                                                <img src="{{getImage('assets/images/user/profile/'. auth()->user()->image, '350x300')}}" alt="{{ __('locale.User')}}">
                                            </div>
                                        </div>
                                        <div class="col-10">
...
@push('script')
    <script>
        ...
        function extraTicketAttachment() {
            $("#fileUploadsContainer").append('<input type="file" name="attachments[]" class="overflow-hidden form-control  mb-2" />')
        }
    </script>
@endpush
