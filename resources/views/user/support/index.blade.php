...
<div class=" my-2 text-end">
    <a href="{{route('ticket.open')}}" class="btn btn-primary btn-sm">{{ __('locale.New Ticket')}}</a>
</div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @forelse($supports as $key => $support)
                            <tr>
                                <td data-label="{{ __('locale.Subject')}}"> <a href="{{ route('ticket.view', $support->ticket) }}" class="fw-bold">[{{ __('locale.Ticket')}}#{{ $support->ticket }}] {{ __($support->subject) }} </a></td>
                                ...
                                <td data-label="{{ __('locale.Last Reply')}}">{{ \Carbon\Carbon::parse($support->last_reply)->diffForHumans() }} </td>
