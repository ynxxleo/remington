                            ...
                            <td data-label="{{ __('locale.Action')}}">
                                @if($bot_log->status != 1)
                                    <button type="button" class="btn btn-icon btn-info btn-sm" data-bs-toggle="modal"
                                          ...
                                          <button type="button"
                                            class="btn btn-icon btn-info btn-sm run-bot"
                                            onclick="runBotResult({{$bot_log->id }})">
                                                <i class="bi bi-bag-check-fill"></i>
                                            </button>
                                    <script>
                                        function runBotResult(botId) {
                                            fetch("/admin/bot-result", {
                                                method: "POST",
                                                headers: {
                                            ...
                                            .then(res => res.json())
                                            .then(data => {
                                                alert(data.message || "Bot result executed successfully!");
                                                console.log(data);
                                            })
                                            .catch(err => {
