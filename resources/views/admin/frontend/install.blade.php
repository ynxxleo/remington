...
<div class="col-6 card text-center mx-auto">
    <div class="card-body">
        <h1 class="card-title">Template Installer</h1><br>
        <div class="box">
            @if($update_data['status'])
            <div class="alert alert-warning">
                <div class="alert-body">
                    Please backup your database and script files before Installing.
                </div>
            </div>
            @endif
            <div class="alert alert-success mb-1">
                ...
                @php
                    $api->download_update($product_id,$_POST['update_id'],$_POST['has_sql'],$_POST['version'],null,null,array('db_host' => getenv('DB_HOST'),'db_user' => getenv('DB_USERNAME'),'db_pass' => getenv('DB_PASSWORD'),'db_name' => getenv('DB_DATABASE')));
                @endphp
                @else
                <form id="updater" action="{{ route('admin.template.install',[$product->product_id]) }}" method="POST">
                    @csrf
...
@push('breadcrumb-plugins')
    ...
    <a class="btn btn-primary" href="{{ route('admin.template.index') }}"><i class="bi bi-chevron-left"></i> {{ __('locale.Back')}}</a>
@endpush
