                                    ...
                                    <div class="list-content justify-content-between">
                                        ...
                                        <div class="d-flex guttar-10px">
                                            <div class="action-btn">
                                                <a class="btn btn-xs btn-icon btn-circle btn-light-alt et-item" data-bs-toggle="modal" data-bs-target="#{{ $template->slug }}" data-slug="{{ $template->slug }}" data-id="{{ $template->id }}" ><i class="bi bi-pencil-square"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <div class="modal fade text-start  w-100" aria-hidden="true" role="dialog" id="{{ $template->slug }}" tabindex="-1">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title">Edit {{ $template->name }} Template</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="card-body">
                                                <form action="{{ route('admin.settings.email.template.update') }}" class="validate-modern" method="POST" id="update_et">
                                                    @csrf
...
<div id="mailSetting" class="modal fade text-start" tabindex="-1" aria-hidden="true" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="card-body">
                <form action="{{ route('admin.settings.email.update') }}" autocomplete ="false" method="POST" id="email_settings">
                    @csrf
                        <h5 class="text-primary">Choose Email Driver </h5>
                        <div class="row">
                            <div class="col-4">
                                <div class="input-item input-with-label">
                                    <input id="smtp_uchk" class="form-check-input" type="radio" {{ email_setting('driver', env('MAIL_DRIVER', 'sendmail')) == 'sendmail' ? 'checked' : '' }} name="site_mail_driver" value="sendmail">
                                    <label for="smtp_uchk">Send Mail</label>
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="input-item input-with-label">
                                    <input id="smtp_chk" class="form-check-input" type="radio" {{ email_setting('driver', env('MAIL_DRIVER', 'smtp')) == 'smtp' ? 'checked' : '' }} name="site_mail_driver" value="smtp">
                                    <label for="smtp_chk">SMTP</label>
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="input-item input-with-label">
                                    <input id="mail" class="form-check-input" type="radio" {{ email_setting('driver', env('MAIL_DRIVER', 'mail')) == 'mail' ? 'checked' : '' }} name="site_mail_driver" value="mail">
